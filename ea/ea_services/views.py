...
class PublishedView(View):
    ...
    def put(self, request, dataset_name):
        ...
        try:
            ...
            # for put, remove ALL datasets from the list and substitute the new
            # one given
    ...
    def post(self, request, dataset_name):
        ...
        try:
            ...
            # for post, remove ALL datasets from the list and substitute the
            # new one given
            published_values = published
    ...
    def get(self, request, dataset_name):
        ...
        try:
            ...
            # note: da[0] is the only record in the filtering of the
            # Dataset_Access objects
            published = []
...
class VariablesView(View):
    ...
    def post(self, request, dataset_name):
        ...
        # for post, remove ALL datasets from the list and substitute the new
        # one given
...
class Dataset_AccessView(View):
    ...
    def put(self, request, group_name):
        ...
        # if the input json data is blank AND there is no da object, create the
        # da object and just return blank
        if dataset == '' and not da:
            # print 'if the input json data is blank AND there is no da object,
            # create the da object and just save the blank'
            new_dataset_list = dataset
            dataset_access_record = Dataset_Access(
                group_name=group_name,
                dataset_list=new_dataset_list
        ...
        # if the input json data is blank AND there is a da object, save the da
        # object
        elif dataset == '' and da:
            print 'if the input json data is blank AND there is a da object, save nothing'

        # if the input json data is not blank AND there is no da object, create
        # the da object and populate with data
        elif dataset != '' and not da:
            # print 'if the input json data is not blank AND there is no da
            # object, create the da object and populate with data'
        ...
        # if the input json data is not blank AND there is a da object, union
        # the data with old data
        else:
            # print 'if the input json data is not blank AND there is a da
            # object, union the data with old data'

            new_dataset_entries = set(dataset.split(','))
            old_dataset_entries = set(da[0].dataset_list.split(','))
            da.delete()
            refined_dataset_list = old_dataset_entries.union(
                new_dataset_entries)
...
class PackagesView(View):
    ...
    def post(self, request, dataset_name):
        ...
        try:
            ...
            new_dataset_list = ''
            if da:
                # delete the record and rewrite the record with the new dataset
                # list
                da.delete()
